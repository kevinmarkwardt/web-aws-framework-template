"""LinkKeeper Report Generator — monthly PDF reports for Pro users.
...
REPORT_BUCKET = os.environ.get("REPORTS_BUCKET", "linkkeeper-reports")
SES_FROM_EMAIL = os.environ.get("SES_FROM_EMAIL", "reports@linkkeeper.co")
...
def _generate_pdf(
    ...
    # Title
    story.append(Paragraph(f"LinkKeeper Portfolio Health Report", title_style))
    ...
    story.append(Paragraph(
        f"Generated by LinkKeeper on {now.strftime('%B %d, %Y')}",
...
def _send_report_email(email: str, pdf_bytes: bytes, report_month: str):
    ...
    msg = MIMEMultipart()
    msg["Subject"] = f"LinkKeeper Monthly Report — {report_month}"
    msg["From"] = SES_FROM_EMAIL
    msg["To"] = email

    body_text = (
        f"Your LinkKeeper Portfolio Health Report for {report_month} is attached.\n\n"
    ...
    attachment.add_header(
        ...
        filename=f"linkkeeper-report-{report_month.lower().replace(' ', '-')}.pdf",
    )
